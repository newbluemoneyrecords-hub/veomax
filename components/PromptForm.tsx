...
const ImageUpload: React.FC<{
...
}> = ({onSelect, onRemove, image, label, className = "w-28 h-20", disabled = false, disabledMessage = "This upload is currently disabled."}) => {
  ...
  if (image) {
    return (
      <div className={`relative group ${className} ${disabled ? 'opacity-50' : ''}`}>
        <img
    ...
    <button
      ...
      className={`${className} ${
        disabled
          ...
          : 'bg-gray-700/50 hover:bg-gray-700 border-2 border-dashed border-gray-600 rounded-lg flex flex-col items-center justify-center text-gray-400 hover:text-white transition-colors'
      }`}>
      <PlusIcon className="w-6 h-6" />
      <span className="text-xs mt-1 text-center px-1">{disabled ? disabledMessage : label}</span>
      <input
        type="file"
        ref={inputRef}
...
const PromptForm: React.FC<PromptFormProps> = ({
  ...
  const [consistentCharacterImage, setConsistentCharacterImage] = useState<ImageFile | null>(
    initialValues?.consistentCharacterImage ?? null,
  );
  ...
  const [duration, setDuration] = useState(initialValues?.duration ?? 4);
  ...
  useEffect(() => {
    if (initialValues) {
      ...
      setConsistentCharacterImage(initialValues.consistentCharacterImage ?? null);
      setInputVideo(initialValues.inputVideo ?? null);
      setInputVideoObject(initialValues.inputVideoObject ?? null);
      setDuration(initialValues.duration ?? 4);
  ...
  const handleSelectMode = (mode: GenerationMode) => {
    ...
    setReferenceImages([]);
    setStyleImage(null);
    setConsistentCharacterImage(null);
    setInputVideo(null);
          ...
          <div className="w-full border-t border-gray-700 pt-4 flex flex-col items-center">
            <div className="flex justify-center gap-4 w-full">
              <div className="flex flex-col items-center">
                ...
                <ImageUpload
                  label={consistentCharacterImage ? "" : "Add Character"}
                  image={consistentCharacterImage}
                  onSelect={setConsistentCharacterImage}
                  onRemove={() => setConsistentCharacterImage(null)}
                  className="w-24 h-16"
                  disabled={!!styleImage}
                  disabledMessage="Clear Style Image"
                />
              </div>
              <div className="flex flex-col items-center">
                <label className="text-xs font-semibold text-gray-400 uppercase tracking-widest block mb-3 text-center">
                ...
                <ImageUpload
                  ...
                  className="w-24 h-16"
                  disabled={!!consistentCharacterImage}
                  disabledMessage="Clear Character"
                />
              </div>
            </div>
          </div>
